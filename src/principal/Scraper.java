...
public class Scraper implements Job {
	...
	public void execute(JobExecutionContext jeContext) throws JobExecutionException {
		...
		//posso mettere questo a false e non da pi√π l'eccezione 
		...
		// salvo tutte le pagine della ricerca in formato html, dopodich√® in uno step successivo ne far√≤ lo scraping
		...
		//crea file di testo dove salver√≤ tutti i risultati dell'attuale ricerca
		...
		while (new File("Page" + x + "AsXml.html").exists()){
			...
			//salvo gli element appropriati all'interno delle liste di elements (prendo gli elementi dalla pagina html) da cui poi prender√≤ le informazioni che mi servono
			flights = doc.getElementsByClass("airline");
			//Il formato che ottengo con go Ë ORA AEROPORTO per l'andata
			depart = doc.select("div.depart");
			//Il formato che ottengo con go Ë ORA AEROPORTO per il ritorno
			...
			for(i=0; i<flights.size(); i++){
				...
				//SCALI
				//Il formato della stringa per gli scali puÚ essere
				...
				stopsGo = "";				
				//Nel caso in cui c'Ë almeno uno scalo vai a prendere dalla terza parola in poi
				...
				if(otheragencies.get(i).select("a").size()!=0){
					for (j = 0; j < otheragencies.get(i).select("a").size(); j++) {
						...
						if(lenghtWords > 2){
							...
							//Ci puÚ essere il caso in cui il nome di un aagenzia sia composto da diverse parole divise da uno spazio
							//Il formato della stringa Ë NOME_OTA PREZZO Ä
